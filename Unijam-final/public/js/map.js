...
function displayMap() {
    ...
    // alert 
    var message = ["Your friend Shivam is attending the Trade show today.","Your friend Rio is attending the Trade show today.","Your friend Sharon is attending the Trade show today."];
    ...
    if (window.confirm(message[a]))
    {
        const eventData = {
            ...
            attendees: "<img src='Images/attendees-plus.png' />",
        ...
        // map.setCenter(153.0, -27.5, 20);
        // map.setZoom(20);


        openMapPopup(eventData.name, eventData.dateTime, eventData.venue, eventData.attendees, eventData.eventPoster, eventData.mainImage, eventData.imageFiles);
    }
...
function mapBookEvent() {
    if (popupBookButton.classList.contains("active")) {
...
//         console.log(eventMarker.name);
//     });
//     console.log("attendee event: ", attendeeEvent);
